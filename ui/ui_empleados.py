...
class EmpleadosFrame(ttk.Frame):
  ...
  def __init__(self, master, usuario_actual=None, *args, **kwargs):
    ...
    self.empleado_seleccionado = None  # Inicializamos el empleado seleccionado

    # Validar si el usuario actual es administrador o rol con permisos
    self.es_admin = False
    ...
    # Si NO es administrador, se restringe el acceso de inmediato
    ...
    # Inicializar estilos, interfaz y carga de datos
  ...
  def _build_ui(self):
    ...
    # CORRECCIÓN CLAVE: Hacer que el propio frame principal expanda su contenido al 100%
    ...
    # Evento para detectar la selección de una fila en la tabla
    ...
    # Poblar campos del panel de datos personales
    campos_personales = [
    ...
    for label_text, var, row, col, tipo in campos_personales:
      ...
      if tipo == "combo":
        ...
        cb.grid(
            row=row, column=col + 1, sticky="w", padx=(0, 10), pady=4, ipady=2
        )
      else:
        entry = tk.Entry(
            frame_personales,
    ...
    # Poblar campos del panel de parámetros de nómina
    campos_nomina = [
