...
def registrar_venta(
    ...
    try:
        ...
        puntos = int(
            total_venta / 10000
        )  # Ejemplo: 1 punto por cada unidad monetaria base

        # 1. Procesar items y descontar stock del inventario
        items_procesados = []
        ...
        # 3. Guardar factura en MongoDB
        coleccion_facturas.insert_one(factura.to_dict())

        # 4. Registrar automáticamente en Contabilidad como INGRESO
        registrar_transaccion(
            tipo="INGRESO",
            categoria="Ventas POS",
            monto=total_venta,
            descripcion=f"Venta Factura {num_fac} - Cliente: {factura.cliente_nombre}",
